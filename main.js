...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateTacticalFingerprint() {
        ...
        ctx.textBaseline = "top"; ctx.font = "14px 'Arial'"; ctx.fillText("MM_v8.3.0", 2, 2);
    ...
    function showConfirm(title, message, isEdit = false, fenceId = null) {
        return new Promise((resolve) => {
            ...
            if (isEdit) {
                msgEl.innerHTML = `
                     <div class="version-tag">v8.3.0-PRO</div>
    ...
    if ('serviceWorker' in navigator && (window.location.protocol === 'https:' || window.location.hostname === 'localhost')) {
        ...
        navigator.serviceWorker.register('sw.js?v=8.3.0').then(reg => {
            reg.onupdatefound = () => {
        ...
        try {
            if (userLocationMarker) {
                ...
                if (!zoneError && zoneUsers) {
                    ...
                    const hasFresh = zoneUsers.some(u => (u.age_secs || 0) <= 15);
                    const hasAllies = zoneUsers.length > 0;
                    
                    let ringColor = 'rgba(15, 23, 42, 0.9)'; // Default Empty
                    if (hasAllies) ringColor = hasFresh ? '#10b981' : '#f59e0b'; // Green if fresh, Amber if stale
