...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateTacticalFingerprint() {
        ...
        ctx.textBaseline = "top"; ctx.font = "14px 'Arial'"; ctx.fillText("MM_v3.6.5", 2, 2);
    ...
    function showConfirm(title, message, isEdit = false, fenceId = null) {
        return new Promise((resolve) => {
            ...
            if (isEdit) {
                msgEl.innerHTML = `
                     <div class="version-tag">v3.6.5-PRO</div>
    ...
    if ('serviceWorker' in navigator) {
        ...
        navigator.serviceWorker.register('sw.js?v=3.6.3').then(reg => {
            reg.onupdatefound = () => {
                const nw = reg.installing;
                nw.onstatechange = () => {
                    if (nw.state === 'installed' && navigator.serviceWorker.controller) {
                        // Skip waiting is handled in sw.js automatically
                    }
                };
            };
        });
